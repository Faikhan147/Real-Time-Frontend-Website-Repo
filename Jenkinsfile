...
stage('Create SonarQube Project') {
    steps {
        withCredentials([string(credentialsId: 'Sonar-Global-Token', variable: 'SONAR_TOKEN')]) {
            sh """
                curl -u ${SONAR_TOKEN}: -X POST "http://13.211.158.194:9000/api/projects/create?name=${SONAR_PROJECT_NAME}&project=${SONAR_PROJECT_KEY}"
            """
        }
    }
}
                    ...
                    dir('Website') {
                        script {
                            ...
                            sh """
                                ...
                                -Dsonar.sources=. \
                                -Dsonar.host.url=http://13.211.158.194:9000
                            """
                        }
                    }
                }
